...
function sustituir(exp, caracter) {
  try {
    const rangos = SpreadsheetApp.getActiveSheet().getActiveRangeList().getRanges();  
    rangos.forEach(rangoActual => rangoActual.setValues(

      rangoActual.getValues().map(fila => fila.map(celda => typeof celda === 'string' ? celda.replace(exp, caracter) : celda))
...
// Funciones invocadas desde el menú que usan la función principal 'sustituir'
